...
public class JSMDataSource extends JSignalMonitorDataSourceAdapter {
    ...
    private float ecgRangeMinimum = -20, ecgRangeMax = 20,
    heartRateRangeMinimum = 0, heartRateRangeMax = 120;
    private float[] ecg;
    ...
    public float[] getChannelData(String signalName, long firstValue,
        ...
        if (firstValue >= lastValue) {
            return new float[0];//este m�todo siempre debe devolver array
        }
        float[] datos;
        float[] fuente;
        int pos1, pos2;
        if (signalName.equals("ECG")) {
            pos1 = TimePositionConverter.timeToPosition(timeOriginDate.getTime(), firstValue,
                    ecgSamplingRate);
            pos2 = TimePositionConverter.timeToPosition(timeOriginDate.getTime(), lastValue,
                    ecgSamplingRate);
    ...
    public float getChannelValueAtTime(String signalName, long time) {
        int pos1 = TimePositionConverter.timeToPosition(timeOriginDate.getTime(), time,
                ecgSamplingRate);
        float dato;
